...
- `popup.html`、`side-panel.html` 等扩展运行页面不得直接新增中文界面文案；优先使用现有 i18n，Clip Note 专属动态文案通过 TypeScript 的 `textContent` 写入。生产构建启用 `ascii_only`，避免 HTML 解码造成乱码。
- 修改中文文案后必须同时检查源码和 `extension/dist/`，不得出现 `å½`、`å‰`、`è§`、`é¢`、`æ²`、`æœ`、`å­—`、`å¹` 等 UTF-8 被错误解码的特征串。

## 弹窗与嵌入式一致性

- `extension/src/popup.html` 和 `extension/src/side-panel.html` 共用 `popup.js`。新增 Clip Note 控件时必须在两个入口保持相同 ID 和结构，业务逻辑只写在共享 TypeScript 控制器中。
- 修改 Clip Note 面板后必须验证弹出窗口和嵌入式构建产物，不得只检查其中一个入口。
